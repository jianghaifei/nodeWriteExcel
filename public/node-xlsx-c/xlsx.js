...
(function make_xlsx(XLSX) {
        ...
        var write_num = (function make_write_num() {
            ...
            function write_num_exp(fmt, val) {
                ...
                if (fmt.match(/^#+0.0E\+0$/)) {
                    ...
                    var ee =
                        Math.floor(Math.log(Math.abs(val)) * Math.LOG10E) %
                        period;
                    if (ee < 0) ee += period;
                    ...
                    if (o.indexOf("e") === -1) {
                        var fakee = Math.floor(
                            Math.log(Math.abs(val)) * Math.LOG10E
                        );
                        if (o.indexOf(".") === -1)
                            o =
                                o[0] +
                ...
                if ((r = fmt.match(dec1)) !== null) {
                    o = rnd(val, r[1].length)
                        ...
                        .replace(/\.(\d*)$/, function ($$, $1) {
                            return (
                                "." + $1 + fill("0", r[1].length - $1.length)
                            );
                        });
                    return fmt.indexOf("0.") !== -1
                        ? o
                        : o.replace(/^0\./, ".");
                }
                fmt = fmt.replace(/^#+([0.])/, "$1");
        ...
        switch (t) {
            case "dbcs":
                ...
                if (has_buf && Buffer.isBuffer(this))
                    o = this.slice(this.l, this.l + 2 * size).toString(
                        "utf16le"
                    );
                else
                    for (i = 0; i != size; ++i) {
    ...
    function write_core_props(cp, opts) {
        ...
        if (opts && opts.Props) {
            ...
            if (opts.Props.subject)
                o[o.length] =
                    "<dc:subject>" + opts.Props.subject + "</dc:subject>";
            if (opts.Props.creator)
                o[o.length] =
                    "<dc:creator>" + opts.Props.creator + "</dc:creator>";
            if (opts.Props.keywords)
                o[o.length] =
                    "<cp:keywords>" + opts.Props.keywords + "</cp:keywords>";
            if (opts.Props.description)
                o[o.length] =
                    "<dc:description>" +
                    opts.Props.description +
                    "</dc:description>";
        }
        if (cp) {
    ...
    function parse_TypedPropertyValue(blob, type, _opts) {
        ...
        switch (type === VT_VARIANT ? t : type) {
            ...
            case 0x50 /*VT_STRING*/:
                return parse_VtString(blob, t, !opts.raw && 4).replace(
                    chr0,
                    ""
                );
            case 0x51 /*VT_USTR*/:
            ...
            if (PIDSI) {
                ...
                if (piddsi.n == "CodePage")
                    switch (PropH[piddsi.n]) {
                        ...
                        default:
                            throw new Error(
                                "Unsupported CodePage: " + PropH[piddsi.n]
                            );
                    }
            } else {
                if (Props[i][0] === 0x1) {
                ...
                case "<color":
                    ...
                    if (
                        ...
                        font.color.rgb = rgb_tint(
                            themes.themeElements.clrScheme[font.color.theme]
                                .rgb,
                            font.color.tint || 0
                        );
                    }
    ...
    function insertCommentsIntoSheet(sheetName, sheet, comments) {
        comments.forEach(function (comment) {
            ...
            if (!cell) {
                ...
                var range = safe_decode_range(
                    sheet["!ref"] || "BDWGO1000001:A1"
                );
        ...
        return function parse_ws_xml_data(sdata, s, opts, guess) {
            ...
            for (
                ...
                for (
                    ...
                    if (!tag.r)
                        tag.r = utils.encode_cell({ r: tagr - 1, c: tagc });
                    d = x.substr(i);
                    p = { t: "" };
    ...
    function write_ws_xml(idx, opts, wb) {
        ...
        var sheetView = writextag("sheetView", null, {
            ...
            tabSelected:
                opts.tabSelected === undefined ? "0" : opts.tabSelected,
            workbookViewId:
            ...
            switch (Rn[3]) {
                case "Data":
                    ...
                    if (Rn[1] === "/")
                        parse_xlml_data(
                            ...
                            ss,
                            dtag,
                            state[state.length - 1][0] == "Comment"
                                ? comment
                                : cell,
                            { c: c, r: r },
                            styles,
                ...
                default:
                    ...
                    if (!state[state.length - 1][1])
                        throw (
                            "Unrecognized tag: " + Rn[3] + "|" + state.join("|")
                        );
                    if (
                    ...
                    case "ShrFmla":
                        {
                            ...
                            /* TODO: capture range */
                            shared_formulae[encode_cell(last_formula.cell)] =
                                val[0];
                        }
                        break;
                    case "LabelSst":
        ...
        return {
            ...
            _addXf: function (attributes) {
                ...
                if (attributes.alignment) {
                    ...
                    if (attributes.alignment.horizontal) {
                        $alignment.attr(
                            "horizontal",
                            attributes.alignment.horizontal
                        );
                    }
                    ...
                    if (attributes.alignment.readingOrder) {
                        $alignment.attr(
                            "readingOrder",
                            attributes.alignment.readingOrder
                        );
                    }
                    if (attributes.alignment.wrapText) {
                        $alignment.attr(
                            "wrapText",
                            attributes.alignment.wrapText
                        );
                    }
                    if (attributes.alignment.textRotation != undefined) {
                        $alignment.attr(
                            "textRotation",
                            attributes.alignment.textRotation
                        );
                    }
            ...
            _addFont: function (attributes) {
                ...
                if (attributes.vertAlign) {
                    $font.append(
                        XmlNode("vertAlign").attr("val", attributes.vertAlign)
                    );
                }
                ...
                if (attributes.color) {
                    if (attributes.color.theme) {
                        ...
                        $font.append(
                            XmlNode("color").attr("rgb", attributes.color.rgb)
                        );
                    }
                }
                ...
                if (attributes.fgColor) {
                    ...
                    if (attributes.fgColor.rgb) {
                        if (attributes.fgColor.rgb.length == 6) {
                            attributes.fgColor.rgb =
                                "FF" + attributes.fgColor.rgb; /// add alpha to an RGB as Excel expects aRGB
                        }
            ...
            _addBorder: function (attributes) {
                ...
                directions.forEach(function (direction) {
                    $border.append(
                        self._getSubBorder(direction, attributes[direction])
                    );
                });
